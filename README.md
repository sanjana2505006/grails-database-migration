...
The integration spec (`DatabaseMigrationIntegrationSpec`) asserts on a **clean Testcontainers PostgreSQL database** that Liquibase tracking tables exist, `person` and `address` have the expected schema (via JDBC `DatabaseMetaData`), and GORM persistence works after migrations apply.
...
If `./grailsw` is unavailable, use Gradle:
